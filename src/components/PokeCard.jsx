...
export function PokeCard({ pokeData, permanentWidht }) {
    const { modalOn } = useContext(ModalContext)
    const pokemon = useGetPokeData(pokeData)

    return (
        <div className={`relative flex flex-col justify-center content-center ${permanentWidht && 'md:w-1/5'} m-4 bg-gradient-to-br from-slate-100 to-slate-400 p-2 shadow-xl rounded-md group`} onClick={() => modalOn(pokemon)}>
